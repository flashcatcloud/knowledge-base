...
9. **Labels是json格式的，如何提取其中的某一个Key？**

  ```
  // 提取rule_note中的detail_url
  {{jsonGet .Labels.rule_note "detail_url"}}

  // 数组格式的提取方法
  {{jsonGet .Labels.rule_note.0 "name"}}

  // 根据条件匹配
  {{jsonget .Labels.rule_note.#(name=="duty").type}}

  ```
  - 更多用法可以参考 [gjson.dev](https://gjson.dev/)
