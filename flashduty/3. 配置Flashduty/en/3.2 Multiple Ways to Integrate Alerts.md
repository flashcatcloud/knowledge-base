...
Access path for dedicated integration: **Channel Details => Integrations => Dedicated Integration**.
- Dedicated integrations can be added when creating a channel or within an existing channel.
- Alerts received through dedicated integrations **do not require routing rules, as all alerts are distributed to that channel** (excluding filtered alerts) and cannot be shared with other channels.
- Choose dedicated integration when alerts don't need to be distributed to multiple channels.
