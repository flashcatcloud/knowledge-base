...
> [!WARNING]
> The Zoom Incoming Webhook app instance is bound to the Zoom account of the user who created it. When that user's Zoom account is deleted or deactivated, Zoom will automatically clean up all app instances installed by that user, including the Incoming Webhook. As a result, the previously generated `Endpoint` address (Webhook URL) will become **immediately invalid**, and Flashduty will no longer be able to send notifications to that Zoom channel.
>
> **Recommendation**: Use a shared team account or service account to create the Incoming Webhook connection, to avoid notification disruptions caused by individual account changes. If the creator's account has been deleted, another member must reinstall the Incoming Webhook app and reconfigure the Webhook URL and Verify Token in Flashduty.
