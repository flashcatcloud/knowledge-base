...
2. Filter for requests to `rum-intake.flashcat.cloud`.
...
Some ad blockers may block the RUM SDK. Try disabling ad blockers or adding exceptions for your domain and `rum-intake.flashcat.cloud`.
...
3. **Use Custom Action Tracking**: For complex interactions, use custom action tracking:
   ...
   ```javascript
   RumBrowser.addAction('click', 'Custom Button Click');
   ```
...
3. **Use Session Replay Wisely**: Session replay can be resource-intensive. Consider using a lower sample rate for session replay:

   ```javascript
   RumBrowser.init({
     // ... other configuration options
     replaySampleRate: 10 // Record session replay for 10% of sessions
   });
   ```
...
3. **Update CSP for Worker Source** (if using session replay):
...
If you need to force a session to be recorded for testing:
...
3. Filter for requests to `rum-intake.flashcat.cloud`
...
2. Contact support through one of these channels:
   ...
   - Support portal: https://support.flashcat.cloud
...
A: Yes, the RUM SDK works in modern mobile browsers on iOS and Android. However, some features like session replay may have limitations on mobile devices.
