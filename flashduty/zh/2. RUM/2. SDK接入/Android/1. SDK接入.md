...
在控制台的[应用管理](https://console.flashcat.cloud/rum/apps)页面：
...
3. **添加拦截器**：配置 Flashcat 提供的拦截器以追踪 OkHttp 请求（目前采用的社区 Datadog 提供的 http 拦截器）。
...
4. **自动创建资源和 Span**：使用 `DatadogInterceptor` 作为拦截器后，OkHttpClient 处理的每个请求都会被自动记录为资源，相关信息（URL、方法、状态码、错误）会自动填充。**注意**：只有在视图处于活动状态时发起的网络请求才会被追踪。要追踪应用在后台时的请求，请参阅 [追踪后台事件](#追踪后台事件)。

5. **追踪网络重定向或重试**：要监控网络重定向或重试，可以将 `DatadogInterceptor` 用作网络拦截器：
...
1. **访问控制台**：登录 控制台，进入对应的 RUM 应用，查看是否有数据上报
2. **触发测试事件**：
   - 打开应用的不同页面，验证页面浏览事件
